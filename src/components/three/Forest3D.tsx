...
export default function Forest3D({
  ...
  return (
    ...
    >
      {reduce || !ready ? (
        // Nền tĩnh thay thế: nắng vàng + sương xanh
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_15%,rgba(255,240,184,0.55),transparent_60%),radial-gradient(circle_at_80%_70%,rgba(169,198,174,0.45),transparent_60%)]" />
      ) : (
        <ForestCanvas density={effective} frameloop={inView ? "always" : "never"} />
      )}
    </div>
  );
}
