...
export default function Rsvp() {
  ...
  return (
    <section id="rsvp" className="relative overflow-hidden bg-gradient-to-b from-rose-petal/40 to-cream py-24 sm:py-28">
      <div className="container-wed">
        ...
        <Reveal className="mx-auto mt-6 max-w-2xl text-center">
          <div className="grid gap-2 font-serif text-lg text-muted sm:grid-cols-2">
            {events.map((ev) => (
              <p key={ev.key}>
                <span className="font-semibold text-wine">{ev.label}:</span> {ev.time}h ·{" "}
                {ev.dateText}
              </p>
            ))}
          </div>
            ...
            {message && (
              <p
                className={`text-center font-serif text-lg ${
                  status === "success" ? "text-wine" : "text-red-600"
                }`}
              >
                {message}
              </p>
