...
export default function EventsSection({ withTitle = true }: { withTitle?: boolean }) {
  return (
    <section id="events" className="section bg-gradient-to-b from-transparent to-sage-pale/40">
            ...
            return (
              ...
              >
                <Reveal direction={reversed ? "left" : "right"} className={reversed ? "lg:order-2" : ""}>
                  <TiltCard max={8} className="overflow-hidden rounded-3xl shadow-soft">
                    <div className="relative aspect-[4/3] w-full">
                      ...
                      <span className="absolute left-5 top-5 rounded-full bg-cream/90 px-4 py-1.5 font-sans text-xs font-semibold uppercase tracking-widest text-ink">
                        {e.label}
                      </span>
                    </div>
                  </TiltCard>
                ...
                <Reveal direction={reversed ? "right" : "left"} className={reversed ? "lg:order-1" : ""}>
                  <p className="eyebrow text-ink">{e.title}</p>
                  <h3 className="mt-2 font-display text-3xl text-ink-deep sm:text-4xl">
                    {e.time}h · {e.dateText}
                  </h3>
